...
export function BulkUploadModal({ open, onOpenChange, onSuccess }: BulkUploadModalProps) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await fetch('/api/rooms/bulk-upload-template', {
        credentials: 'include',
      });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/rooms/bulk-upload', {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV file to add multiple rooms at once. You can also map tenants to rooms.
          </DialogDescription>
        </DialogHeader>
        ...
        {step === 'processing' && (
          <div className="py-12 space-y-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {dryRun ? 'Validating rooms...' : 'Creating rooms...'}
              </p>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-sm ${result.success ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
                    {result.summary.created} of {result.summary.total} rooms {result.dryRun ? 'validated' : 'created'}
                    ...
                    {result.warnings.map((warning, idx) => (
                      <div key={idx} className="text-sm p-2 bg-amber-50 dark:bg-amber-950/30 rounded border border-amber-200 dark:border-amber-800">
                        <span className="font-medium text-amber-700 dark:text-amber-300">
                          Row {warning.row} - {warning.roomNumber}:
                        </span>{' '}
